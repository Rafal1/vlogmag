...
?>
                ...
                <a href="http://vlogmag.pl/kontakt"><img src="/wp-content/themes/profitmag/images/sendVideo.jpg"><a/>
                </div>
                </div><!-- .beides-block -->
<?php    
            }
...
?>
            <ul><li><?php _e( 'Brak powiązanych postów.', 'profitmag'); ?></li></ul>
<?php            
        }
      wp_reset_postdata();          
    }
...
function profitmag_alter_comment_form( $form ) {    
    ...
    $form['fields']['author'] = '<p class="comment-form-author"><label for="author"></label><input id="author" name="author" type="text" placeholder="pseudonim" value="" size="30" '.$req.'/></p>';
    ...
    $form['fields']['url'] = '<p class="comment-form-url"><label for="url"></label> <input id="url" name="url" placeholder="strona internetowa" type="url" value="" size="30" /></p>';
    $form['comment_field'] = '<p class="comment-form-comment"><label for="comment"></label> <textarea id="comment" name="comment" placeholder="Napisz coś..." cols="45" rows="8" aria-required="true"></textarea></p>';
    $form['comment_notes_before'] = '';
    $form['label_submit'] = 'Dodaj komentarz';
    $form['title_reply'] = '<span class="bordertitle-red"></span>Napisz komentarz';
    return $form;
}
...
<?php
			...
			printf( __('%1$s at %2$s', 'profitmag'), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( __( '(Edytuj)','profitmag' ), '  ', '' );
		?>
	</div>
...
function profitmag_search_form( $form ) {
	...
	<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="SZUKAJ" />
